package com.zshnb.codegenerator.constant

class BackendFreeMarkerFileConstant {
    companion object {
        const val ENTITY_TEMPLATE = "ssmp/entity.ftl"
        const val SERVICE_TEMPLATE = "ssmp/service.ftl"
        const val SERVICE_IMPL_TEMPLATE = "ssmp/serviceImpl.ftl"
        const val MAPPER_TEMPLATE = "ssmp/mapper.ftl"
        const val RESTFUL_CONTROLLER_TEMPLATE = "ssmp/restfulController.ftl"
        const val LAYUI_CONTROLLER_TEMPLATE = "ssmp/layuiController.ftl"
        const val LAYUI_INDEX_CONTROLLER_TEMPLATE = "ssmp/layuiIndexController.ftl"
        const val MAPPER_XML_TEMPLATE = "ssmp/mapperXml.ftl"
        const val LIST_REQUEST_TEMPLATE = "ssmp/listRequest.ftl"
        const val LIST_RESPONSE_TEMPLATE = "ssmp/listResponse.ftl"
        const val PAGE_REQUEST_TEMPLATE = "ssmp/pageRequest.ftl"
        const val POM_TEMPLATE = "ssmp/pom.ftl"
        const val SPRING_BOOT_MAIN_APPLICATION = "ssmp/springbootMain.ftl"
        const val RESPONSE_TEMPLATE = "ssmp/response.ftl"
        const val SQL_TEMPLATE = "ssmp/sql.ftl"
        const val APPLICATION_TEMPLATE = "ssmp/application.ftl"
        const val MYBATIS_PLUS_CONFIG_TEMPLATE = "ssmp/mybatisPlusConfig.ftl"
        const val INIT_DATA_TEMPLATE = "ssmp/initData.ftl"
        const val MENU_DTO_TEMPLATE = "ssmp/menuDto.ftl"
        const val LOGIN_REQUEST_TEMPLATE = "ssmp/loginRequest.ftl"
    }
}