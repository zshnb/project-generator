...
import com.github.studyPlatform.service.IDiscussionService;
...
public class DiscussionServiceImpl extends ServiceImpl<DiscussionMapper, Discussion> implements IDiscussionService {
    ...
    public Discussion add(Discussion discussion) {
        save(discussion);
        return getById(discussion.getId());
    }
    ...
    @Override
    public Discussion detail(int id) {
        return getById(id);
    }
    ...
    @Override
    public ListResponse<Discussion> list(ListDiscussionRequest request) {
            IPage<Discussion> page = page(new Page<>(request.getPageNumber(), request.getPageSize()));
